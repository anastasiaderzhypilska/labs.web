export const initialZoosFallback = (filters = {}) => {
  const zoos = [
  ...
  return zoos.filter(z => {
    if (filters.type && z.type !== filters.type) return false;
    if (filters.q && !z.name.toLowerCase().includes(filters.q.toLowerCase())) return false;
    return true;
  });
};
